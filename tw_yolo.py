...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        print(end - start)
        return image,gps_infos

    def close_session(self):
        self.sess.close()


def detect_video(yolo, video_path, output_path="",gps_path=""):
    import cv2
    ...
    isGPSInfo = True if gps_path != "" else False
    csv_writer = None
    if isGPSInfo:
        f = open(gps_path,'w')
        csv_writer = csv.writer(f)
        csv_writer.writerow(['className', 'gps'])
    if isOutput:
    ...
    while True:
        ...
        image,gps_infos = yolo.detect_image(image)
